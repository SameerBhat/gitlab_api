...
$('#repoForm').submit(function (event) {
  ...
  window.location.replace("editor.html?project_id=" + projectId + "&token=" + getToken() + "&file_location=" + file_location);
  
});

$('.create-file').on('click', function(e){
  e.preventDefault();
  var projectId = $('#repoForm').find("option:selected").prop("id");
  var file_location = $('input[name=default_location]').val();
  window.location.replace("creator.html?project_id=" + projectId + "&token=" + getToken() + "&file_location=" + file_location);
...
$('#creator_form').submit(function (e) {
  e.preventDefault();
  var $inputs = $('#creator_form :input');
  var values = {};
  $inputs.each(function () {
    values[this.name] = $(this).val();
  });
  ...
  if (values['file_location'] == "" ||
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        alert("File Created Successfully to :" + result.file_path);
      },
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        console.log(errorThrown)
...
function setUpCodeCreator(projectId, token) {
